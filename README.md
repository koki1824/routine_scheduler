...
## Streamlit Community Cloudに置く場合

GitHubには `credentials.json` と `token.json` をアップロードしません。Streamlit Cloudの `Secrets` に以下の形で貼り付けます。

```toml
google_credentials_json = """
ここに credentials.json の中身を丸ごと貼る
"""

google_token_json = """
ここに token.json の中身を丸ごと貼る
"""
```

`config.json` の保存先はGoogle Sheetsにします。
